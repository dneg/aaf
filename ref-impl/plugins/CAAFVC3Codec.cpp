...
// {3248A265-F8A8-49ea-9859-C4EE0FF83163}
const CLSID CLSID_AAFVC3Codec = 
{ 0x3248a265, 0xf8a8, 0x49ea, { 0x98, 0x59, 0xc4, 0xee, 0xf, 0xf8, 0x31, 0x63 } };
...
//
// {0883D3E9-175D-4629-BB1E-2C1BCA9C908B}
const aafUID_t AVID_VC3_PLUGIN = 
{ 0x883d3e9, 0x175d, 0x4629, { 0xbb, 0x1e, 0x2c, 0x1b, 0xca, 0x9c, 0x90, 0x8b } };
...
CAAFVC3Codec::CAAFVC3Codec (IUnknown * pControllingUnknown)
  : CAAFUnknown (pControllingUnknown)
{
	plugin_trace("CAAFVC3Codec::CAAFVC3Codec() constructor\n");
...
#ifdef USE_VC3_CODEC
	// variables for VC3 coder interface
		_vc3_buffer=NULL;
#endif
	...
	kSupportedFlavours[0].name = L"AAF VC3 Codec (no flavour)";
	kSupportedFlavours[1].name = L"AAF VC3 Codec ID 1235";
	kSupportedFlavours[2].name = L"AAF VC3 Codec ID 1237";
	kSupportedFlavours[3].name = L"AAF VC3 Codec ID 1238";
	kSupportedFlavours[4].name = L"AAF VC3 Codec ID 1241";
	kSupportedFlavours[5].name = L"AAF VC3 Codec ID 1242";
	kSupportedFlavours[6].name = L"AAF VC3 Codec ID 1243";
	kSupportedFlavours[7].name = L"AAF VC3 Codec ID 1250";
	kSupportedFlavours[8].name = L"AAF VC3 Codec ID 1251";
	kSupportedFlavours[9].name = L"AAF VC3 Codec ID 1252";
	kSupportedFlavours[10].name = L"AAF VC3 Codec ID 1253";
...
CAAFVC3Codec::~CAAFVC3Codec ()
{
	plugin_trace("CAAFVC3Codec::~CAAFVC3Codec()\n");
...
void CAAFVC3Codec::SetEssenceStream(IAAFEssenceStream *stream)
{
	plugin_trace("CAAFVC3Codec::SetEssenceStream()\n");
	if (_stream != stream)
	{
		if (NULL != _stream)
			_stream->Release();
...
void CAAFVC3Codec::SetCompressionEnabled(aafCompressEnable_t compEnable)
{
	_compressEnable = (compEnable == kAAFCompressionEnable) ? kAAFCompressionEnable : kAAFCompressionDisable;
}

void CAAFVC3Codec::SetNumberOfSamples(const aafLength_t& numberOfSamples)
{
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::CountDefinitions (aafUInt32 *pDefCount)
{
	plugin_trace("CAAFVC3Codec::CountDefinitions()\n");
	if(NULL == pDefCount)
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetIndexedDefinitionID (aafUInt32 /* index */, aafUID_t *uid)
{
	plugin_trace("CAAFVC3Codec::GetIndexedDefinitionID()\n");
	if(NULL == uid)
		return AAFRESULT_NULL_PARAM;

	*uid = kAAFCodecVC3;		// UID of the VC3 codec definition

	return AAFRESULT_SUCCESS;
}

HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetPluginDescriptorID (aafUID_t *uid)
{
	plugin_trace("CAAFVC3Codec::GetPluginDescriptorID()\n");
	if(NULL == uid)
		return AAFRESULT_NULL_PARAM;

	*uid = AVID_VC3_PLUGIN;		// UID of the PluginDescriptor

	return AAFRESULT_SUCCESS;
}


// The method creates some descriptor properties to handle legacy VC3 
// variations .
HRESULT STDMETHODCALLTYPE CAAFVC3Codec::CreateLegacyPropDefs( 
    IAAFDictionary	*p_dict )
{
	plugin_trace("CAAFVC3Codec::CreateLegacyPropDefs()\n");
    HRESULT		hr = S_OK;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetIndexedDefinitionObject (aafUInt32 index, IAAFDictionary *dict, IAAFDefObject **def)
{
	plugin_trace("CAAFVC3Codec::GetIndexedDefinitionObject()\n");
	HRESULT hr = S_OK;
	...
	try
	{
		...
		// Creates descriptor properties to handle legacy VC3.
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::CreateDescriptor (IAAFDictionary *dict, IAAFPluginDef **descPtr)
{
	plugin_trace("CAAFVC3Codec::CreateDescriptor()\n");
	HRESULT hr = S_OK;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::SetEssenceAccess(IAAFEssenceAccess *access)
{
	plugin_trace("CAAFVC3Codec::SetEssenceAccess()\n");
	if(access == NULL)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::CountFlavours(aafUInt32 *pCount)
{
	plugin_trace("CAAFVC3Codec::CountFlavours()\n");
	if(pCount == NULL)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetIndexedFlavourID (aafUInt32 index,
        aafUID_t *  pFlavour)
{
	plugin_trace("CAAFVC3Codec::GetIndexedFlavourID()\n");
	if (pFlavour == NULL)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::CountDataDefinitions (aafUInt32 *pDefCount)
{
	plugin_trace("CAAFVC3Codec::CountDataDefinitions()\n");
	if (NULL == pDefCount)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetIndexedDataDefinition (aafUInt32  /* index */,
        aafUID_t * pFlavour)
{
	plugin_trace("CAAFVC3Codec::GetIndexedDataDefinition()\n");
	if (NULL == pFlavour)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetMaxCodecDisplayNameLength (
        aafUInt32  *bufSize)
{
	plugin_trace("CAAFVC3Codec::GetMaxCodecDisplayNameLength()\n");
	if (NULL == bufSize)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetCodecDisplayName (aafUID_constref flavour,
        aafCharacter *  pName,
        aafUInt32  bufSize)
{
	plugin_trace("CAAFVC3Codec::GetCodecDisplayName()\n");
	if (NULL == pName)
		return AAFRESULT_NULL_PARAM;
	if (0 >= bufSize)
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::SetFlavour (aafUID_constref flavour)
{
	plugin_trace("CAAFVC3Codec::SetFlavour()\n");
...
{
	plugin_trace("CAAFVC3Codec::CountChannels()\n");
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetSelectInfo (IAAFSourceMob *fileMob,
        IAAFEssenceStream *stream,
        aafSelectInfo_t *  pSelectInfo)
{
	plugin_trace("CAAFVC3Codec::GetSelectInfo()\n");
	HRESULT hr = S_OK;
	...
	try
	{
		...
		checkExpression(AAFRESULT_PROP_NOT_PRESENT == hr || AAFRESULT_SUCCESS == hr, hr);
		if (AAFRESULT_SUCCESS == hr && IsVC3(compressionID) && 0 == padBits)
		{
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::CountSamples (
        aafUID_constref essenceKind,
        aafLength_t *  pNumSamples)
{
	plugin_trace("CAAFVC3Codec::CountSamples()\n");
	if (NULL == pNumSamples)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::ValidateEssence (IAAFSourceMob *fileMob,
        IAAFEssenceStream *stream,
...
{
	plugin_trace("CAAFVC3Codec::ValidateEssence()\n");
...
//
HRESULT STDMETHODCALLTYPE
CAAFVC3Codec::Create (IAAFSourceMob *p_srcmob,
...
{
	plugin_trace("CAAFVC3Codec::Create()\n");
...
//
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::Open (IAAFSourceMob *p_srcmob,
		aafMediaOpenMode_t  openMode,
    IAAFEssenceStream * stream,
    aafCompressEnable_t compEnable)
{
	plugin_trace("CAAFVC3Codec::Open()\n");
    HRESULT hr = S_OK;
...
HRESULT STDMETHODCALLTYPE CAAFVC3Codec::WriteSamples(
	aafUInt32	nSamples,
        aafUInt32	buflen,
...
{
	plugin_trace("CAAFVC3Codec::WriteSamples()\n");
    HRESULT	hr = S_OK;
    aafUInt32	n;
	...
	*pTotalSamplesWritten = 0;
	...
	for( n=0; n<nSamples; n++ )
	{
		if (_compressEnable == kAAFCompressionEnable && IsVC3(_compression))
		{
			...
			comprBuffer=new aafUInt8[MAX_VC3_COMPRESSED_FRAME];

			int FormatNumber;
			switch(_ComprID)
			{
			case 1235: FormatNumber=1;  break;
			case 1237: FormatNumber=2;  break;
			case 1238: FormatNumber=3;  break;
			case 1241: FormatNumber=4;  break;
			case 1242: FormatNumber=5;  break;
			case 1243: FormatNumber=6;  break;
			case 1250: FormatNumber=7;  break;
			case 1251: FormatNumber=8; break;
			case 1252: FormatNumber=9; break;
			case 1253: FormatNumber=10; break;
			default:
				FormatNumber=1;  //make any guess
				break;
			}


			AvidDNxHDCodec * DNxCodec;
			DNxCodec=CreateAvidDNxHDCodec();
			if(!DNxCodec)
				throw HRESULT( AAFRESULT_INVALID_OP_CODEC );
			long result=DNxCodec->EncodeFrame(comprBuffer,buffer,FormatNumber);
			if(result!=DNxHDCODEC_SUCCESSFUL)
				throw HRESULT( AAFRESULT_INVALID_OP_CODEC );
			DestroyAvidDNxHDCodec(DNxCodec);

			sampleBytesWritten = 0;
			checkResult( _stream->Write( GetBytesPerSample(), comprBuffer, &sampleBytesWritten));


			delete[] comprBuffer;

#else
			// Can't compress without VC3 codec
			throw HRESULT( AAFRESULT_INVALID_OP_CODEC );
#endif
		}
		else
		{
			if(_HeaderFilled<VC3HEADERSIZE)
			{
				if(_HeaderFilled==0)
				{
					_TotalBytesWritten=0;
					if(buflen>=VC3HEADERSIZE)
					{
						memcpy(_Header,buffer,VC3HEADERSIZE);
						_HeaderFilled=VC3HEADERSIZE;
...
*/
HRESULT STDMETHODCALLTYPE CAAFVC3Codec::ReadSamples(
	aafUInt32	nSamples,
	aafUInt32	buflen,
	aafDataBuffer_t	buffer,
...
{
	plugin_trace("CAAFVC3Codec::ReadSamples()\n");
    HRESULT	hr = S_OK;
    aafUInt32	n;
	...
	*pTotalBytesRead = 0;
	...
	plugin_trace("CAAFVC3Codec::ReadSamples() sampleSize=%d nSamples=%d sampleSize*nSamples=%d buflen=%d\n",
	...
	for( n = 0; n < nSamples; n++ )
	{
		...
		if (_compressEnable == kAAFCompressionEnable ) // && IsVC3(_compression))
		{
			...
			{ //this must be the first round so get the header and the vital information
				_HeaderFilled=true;
				checkResult( _stream->Read( VC3HEADERSIZE, _Header, &sampleBytesRead ));
				...
				if(IsInterlaced())
				 ...
				 if(_vc3_buffer==NULL)
					 _vc3_buffer=new aafUInt8[GetBytesPerSample()];
			...
			checkExpression( _vc3_buffer != NULL, AAFRESULT_INTERNAL_ERROR );
			

			//get encoded frame
			checkResult( _stream->Read( GetBytesPerSample(), _vc3_buffer, &sampleBytesRead ));
			...
			{//got a full frame
				...
				if(!DNxCodec)
					throw HRESULT( AAFRESULT_INVALID_OP_CODEC );
				long result=DNxCodec->DecodeFrame(_vc3_buffer,buffer);
				if(result!=DNxHDCODEC_SUCCESSFUL)
					throw HRESULT( AAFRESULT_INVALID_OP_CODEC );
				DestroyAvidDNxHDCodec(DNxCodec);
...
*/
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::Seek (aafPosition_t  sampleFrame)
{
	plugin_trace("CAAFVC3Codec::Seek()\n");
    HRESULT	hr = S_OK;
...
HRESULT STDMETHODCALLTYPE CAAFVC3Codec::ReadDescriptor( 
    CAAFCDCIDescriptorHelper&	descriptorHelper )
{
	plugin_trace("CAAFVC3Codec::ReadDescriptor()\n");
    HRESULT		hr = S_OK;
    ...
    try
    {
	// future: GetEssenceElementKey() into _essenceElementKey
...
void CAAFVC3Codec::SetCodecState(void)
{
	// compression should be disabled for non-VC3 data
	if (_compressEnable == kAAFCompressionEnable)
		checkExpression (IsVC3(_compression),AAFRESULT_BADCOMPR);
...
void CAAFVC3Codec::UpdateDescriptor (CAAFCDCIDescriptorHelper& descriptorHelper)
{
	plugin_trace("CAAFVC3Codec::UpdateDescriptor()\n");
	// Update FileDescriptor properties

	// future: SetEssenceElementKey()
...
void CAAFVC3Codec::UpdateCalculatedData( void )
{
	int ALPF;
	int SPL;

	plugin_trace("CAAFVC3Codec::UpdateCalculatedData()\n");
	...
	if( !_UseLegacyAvidUIDs )
	{
		...
		if( kAAFFalse != EqualAUID( &_eek, &NULL_UID ) ) _eek = GC_EEK;
	...
	else
	{
		...
		if( kAAFFalse != EqualAUID( &_eek, &NULL_UID ) ) _eek = Avid_EEK;
...
*/
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::CompleteWrite (IAAFSourceMob *fileMob = NULL)
{
	plugin_trace("CAAFVC3Codec::CompleteWrite()\n");
    HRESULT hr = S_OK;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::CreateDescriptorFromStream (IAAFEssenceStream * pStream,
        IAAFSourceMob *fileMob)
{
	plugin_trace("CAAFVC3Codec::CreateDescriptorFromStream()\n");
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetCurrentEssenceStream (IAAFEssenceStream ** ppStream)
{
	plugin_trace("CAAFVC3Codec::GetCurrentEssenceStream()\n");
	if (NULL == ppStream)
		return AAFRESULT_NULL_PARAM;
	if (NULL == _stream)
...
// 
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::PutEssenceFormat (IAAFEssenceFormat * pFormat)
{
	plugin_trace("CAAFVC3Codec::PutEssenceFormat()\n");
    HRESULT			hr = S_OK;
	...
	for (i = 0; i < numSpecifiers; ++i)
	{
		...
		// TODO: accept kAAFEssenceElementKey

		
		if (EqualAUID(&kAAFLegacyAUIDs, &param.opcode))
		...
		{	// Validate the in-memory size.
			...
			// Only VC3 compressions supported 
			checkExpression (IsVC3(param.operand.expUID), AAFRESULT_BADCOMPR);
...
HRESULT STDMETHODCALLTYPE CAAFVC3Codec::GetEssenceFormat(
	IAAFEssenceFormat	*pTemplate,
	IAAFEssenceFormat	**pResult)
{
	plugin_trace("CAAFVC3Codec::GetEssenceFormat()\n");
    HRESULT			hr = S_OK;
    ...
    *pResult = NULL;
	...
	for( i=0; i<numSpecifiers; i++ )
	{
		...
		// TODO: write out kAAFEssenceElementKey
	    ...
	    {
		// VC3 codec does not transfer separate lines.
	    ...
	    {
		// VC3 codec does not handle compressed data.
		param.operand.expBoolean = kAAFFalse;
		checkResult( ADD_FORMAT_SPECIFIER( 
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetDefaultEssenceFormat(IAAFEssenceFormat **pResult)
{
	plugin_trace("CAAFVC3Codec::GetDefaultEssenceFormat()\n");
	HRESULT			hr = S_OK;
	IAAFEssenceFormat	*p_fmt = NULL;
	aafEssenceFormatData_t	param;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetEssenceDescriptorID (aafUID_t *uid)
{
	plugin_trace("CAAFVC3Codec::GetEssenceDescriptorID()\n");
	if(NULL == uid)
		return AAFRESULT_NULL_PARAM;
...
HRESULT STDMETHODCALLTYPE
    CAAFVC3Codec::GetEssenceDataID (aafUID_t *uid)
{
	plugin_trace("CAAFVC3Codec::GetEssenceDataID()\n");
	if(NULL == uid)
		return AAFRESULT_NULL_PARAM;

	*uid = AUID_AAFEssenceData;			// stored class UID of the VC3 data object

	return AAFRESULT_SUCCESS;
}

HRESULT STDMETHODCALLTYPE CAAFVC3Codec::GetIndexedSampleSize (
    aafUID_constref	dataDefID,
    aafPosition_t	pos,
    aafLength_t		*pResult)
{
	plugin_trace("CAAFVC3Codec::GetIndexedSampleSize()\n");
...
HRESULT STDMETHODCALLTYPE CAAFVC3Codec::GetLargestSampleSize(
    aafUID_constref	dataDefID,
    aafLength_t		*pResult )
{
	plugin_trace("CAAFVC3Codec::GetLargestSampleSize()\n");
...
HRESULT CAAFVC3Codec::InternalQueryInterface
(
    REFIID riid,
    void **ppvObj)
{
...
// 
AAF_DEFINE_FACTORY(AAFVC3Codec)
