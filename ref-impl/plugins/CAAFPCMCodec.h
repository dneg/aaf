...
#ifndef __CAAFPCMCodec_h__
#define __CAAFPCMCodec_h__
...
EXTERN_C const CLSID CLSID_AAFPCMCodec;
...
protected:
  ...
  //
  CAAFPCMCodec (IUnknown * pControllingUnknown);
  virtual ~CAAFPCMCodec ();
...
public:
 ...
 STDMETHOD (CountFlavours)
  ...
  // Given some raw essence (like a PCM file), create
...
private:
	...
	IAAFPCMDescriptor	*_pcmdes;
	IAAFSoundDescriptor	*_snddes;
	IAAFEssenceAccess	*_access;
	IAAFFileDescriptor	*_filedes;
	aafUID_t			_containerFormat;
	aafMobID_t			_mobID;				// file SourceMob::MobID
	aafPosition_t		_origin;			// TimelineMobSlot::Origin
	...
	AAFRESULT fillSwappedPCMData(aafUInt8 **destBufHdl, aafUInt32 maxsize, void *data);
	AAFRESULT scanSwappedPCMData(aafUInt8 **srcBufHdl, aafUInt32 maxsize, void *data);

	// Internal methods
	HRESULT write_BWF_header(void);
	HRESULT write_updated_BWF_size(void);
	HRESULT find_data_chunk_offset(void);

};

#endif // ! __CAAFPCMCodec_h__
