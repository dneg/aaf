...
ImplEnumAAFClassDefs::ImplEnumAAFClassDefs ()
{
	_current = 0;
	_enumObj = NULL;
	_enumProp = NULL;
...
AAFRESULT STDMETHODCALLTYPE
    ImplEnumAAFClassDefs::Next (
      aafUInt32  count,
      ImplAAFClassDef **ppClassDefs,
      aafUInt32 *pFetched)
{
	ImplAAFClassDef**	ppDef;
	aafUInt32			numDefs;
	HRESULT				hr;
...
AAFRESULT STDMETHODCALLTYPE
    ImplEnumAAFClassDefs::Clone (
      ImplEnumAAFClassDefs **ppEnum)
{
	ImplEnumAAFClassDefs	*result;
	AAFRESULT				hr;

	result = (ImplEnumAAFClassDefs *)CreateImpl(CLSID_EnumAAFClassDefs);
	if (result == NULL)
		return E_FAIL;
...
AAFRESULT STDMETHODCALLTYPE
    ImplEnumAAFClassDefs::SetEnumProperty( ImplAAFObject *pObj,
										   classDefWeakRefArrayProp_t *pProp)
{
	if (_enumObj)
	  _enumObj->ReleaseReference();
	_enumObj = 0;
...
AAFRESULT STDMETHODCALLTYPE
    ImplEnumAAFClassDefs::SetEnumStrongProperty( ImplAAFObject *pObj, classDefStrongRefArrayProp_t *pProp)
{
	if (_enumObj)
	  _enumObj->ReleaseReference();
	_enumObj = 0;
