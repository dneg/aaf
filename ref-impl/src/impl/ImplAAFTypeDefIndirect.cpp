      ...
      ImplAAFPropertyValue ** ppIndirectValue)
{
  ...
  *ppIndirectValue = NULL;
  ...
  _typeDefAUID->type()->externalize((OMByte*)&actualTypeID, 
                            _internalAUIDSize,
	...
	pActualType->type()->externalize((OMByte *)pActualValueDataBits,
                           actualInternalSize,
      ...
      ImplAAFPropertyValue ** ppIndirectValue)
{
  ...
  *ppIndirectValue = NULL;
  ...
  _typeDefAUID->type()->externalize((OMByte*)&actualTypeID, 
	...
	// Write the actual value into the indirect value in its external form.
	pActualType->type()->externalize((OMByte *)pInitData,
                           initDataSize,
                           (OMByte*)&pIndirectValueDataBits[_externalIndirectSize],
                           actualExternalSize,
                           originalByteOrder);
      ...
      aafUID_t *typeID)
{
  ...
  if (typeID || ppActualType)
  {
		...
		if (originalByteOrder != hostByteOrder())
		  _typeDefAUID->type()->reorder((OMByte*)&pIndirectValueDataBits[sizeof(originalByteOrder)], _externalAUIDSize);

    // "read" the actual type id from the externalized indirect value.
    _typeDefAUID->type()->internalize ((OMByte*)&pIndirectValueDataBits[sizeof(originalByteOrder)],
						                   _externalAUIDSize,
						                   (OMByte*)&actualTypeID,
						                   sizeof(actualTypeID),
						                   hostByteOrder());
  ...
  if (NULL != ppActualType)
  {
    ...
    if (!ppActualType)
      return AAFRESULT_NULL_PARAM;
		...
		if (originalByteOrder != hostByteOrder())
			(*ppActualType)->type()->reorder(actualExternalBytes, actualExternalBytesSize);

		// Ask the type to compute the internal size of the actual data.
		*actualValueSize = (*ppActualType)->type()->internalSize(actualExternalBytes, actualExternalBytesSize);
      ...
      ImplAAFPropertyValue ** ppActualValue)
{
  ...
  *ppActualValue = NULL;
  ...
  if (AAFRESULT_FAILED(result))
	...
	// First make sure that the actual data is in the correct byte order.
	if (originalByteOrder != hostByteOrder())
		pActualType->type()->reorder(actualExternalBytes, actualExternalBytesSize);

	// Ask the type to internalize into the actual value bytes.
	pActualType->type()->internalize(actualExternalBytes,
...
ImplAAFTypeDefIndirect::identification(void) const
{
  return ImplAAFMetaDefinition::identification();
}

const wchar_t* ImplAAFTypeDefIndirect::name(void) const
{
  return ImplAAFMetaDefinition::name();
}

bool ImplAAFTypeDefIndirect::hasDescription(void) const
{
  return ImplAAFMetaDefinition::hasDescription();
}

const wchar_t* ImplAAFTypeDefIndirect::description(void) const
{
  return ImplAAFMetaDefinition::description();
}

bool ImplAAFTypeDefIndirect::isPredefined(void) const
{
...
bool ImplAAFTypeDefIndirect::isFixedSize(void) const
{
  bool result = false;
...
OMUInt32 ImplAAFTypeDefIndirect::externalSize(void) const
{
  return PropValSize();
}
...
OMUInt32 ImplAAFTypeDefIndirect::internalSize(void) const
{
  return NativeSize();
}
...
void ImplAAFTypeDefIndirect::accept(OMTypeVisitor& visitor) const
{
  visitor.visitIndirectType(this);
  // We don't visit the indirect type here // tjb !!!
}


OMUniqueObjectIdentification ImplAAFTypeDefIndirect::actualTypeId(
    const OMByte* externalBytes, OMUInt32 externalSize) const
{
    TRACE("ImplAAFTypeDefIndirect::actualTypeId");
    PRECONDITION("Object has been initialized", _initialized);
    PRECONDITION("Valid external bytes", externalBytes != 0);
    PRECONDITION("Valid external size", externalSize > 0);
    PRECONDITION("External byte size is large enough for id and byte order", 
    ...
    OMByteOrder originalByteOrder = byteOrder(externalBytes, externalSize);
    
    // use OMUniqueObjectIdentificationType to reorder for us
    OMUniqueObjectIdentificationType idType;
    if (originalByteOrder != hostByteOrder())
    {
	    idType.reorder((OMByte*)&externalBytes[sizeof(originalByteOrder)], _externalAUIDSize);
    }

    // use OMUniqueObjectIdentificationType to internalize for us
    OMUniqueObjectIdentification actualTypeId;
    idType.internalize(&externalBytes[sizeof(OMByteOrder)], _externalAUIDSize,
        (OMByte*)&actualTypeId, sizeof(OMUniqueObjectIdentification), originalByteOrder);
...
OMType* ImplAAFTypeDefIndirect::actualType(const OMByte* externalBytes, 
    OMUInt32 externalSize) const
{
    ...
    OMUniqueObjectIdentification actualTypeID = actualTypeId(externalBytes, 
        externalSize);

    ImplAAFTypeDef* pActualType;
    HRESULT hr = LookupActualType(*(reinterpret_cast<aafUID_t*>(&actualTypeID)), 
        &pActualType);
    ASSERTU(AAFRESULT_SUCCEEDED(hr));
    pActualType->ReleaseReference();
    
    return pActualType->type();    
}
  
OMByteOrder ImplAAFTypeDefIndirect::byteOrder(const OMByte* externalBytes,
    OMUInt32 externalSize) const
{
    TRACE("ImplAAFTypeDefIndirect::byteOrder");
    PRECONDITION("Object has been initialized", _initialized);
    PRECONDITION("Valid external bytes", externalBytes != 0);
    PRECONDITION("Valid external size", externalSize > 0);
    PRECONDITION("External byte size is large enough for id and byte order", 
...
void ImplAAFTypeDefIndirect::actualSize(const OMByte* externalBytes, 
    OMUInt32 externalSize, OMUInt32& actualSize) const
{
...
void ImplAAFTypeDefIndirect::actualData(const OMByte* externalBytes, 
    OMUInt32 externalSize, OMByte* actualBytes, OMUInt32& actualSize) const
{
...
void ImplAAFTypeDefIndirect::externalData(const OMByte* externalBytes, OMUInt32 externalSize,
    const OMByte*& externalDataBytes, OMUInt32& externalBytesSize) const
{
