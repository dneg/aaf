...
static StructuredStorage* _ssStorage = 0;
...
// The file kind for 4K structured storage binary AAF files.
//
const CLSID aafFileKindAaf4KSSBinary =
...
// The signature for 4K structured storage binary AAF files. This includes
// the structured storage file signature.
//
unsigned char aafFileSignatureAaf4KSSBinary[] = {
  0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1,
...
struct format {
  ...
  {aafFileSignatureAaf4KSSBinary,
   sizeof(aafFileSignatureAaf4KSSBinary),
   &aafFileKindAaf4KSSBinary},
                 ...
                 int isRoot)
{
  ...
  do {
    ...
    if (status == S_OK) {
      ...
      //
      _ssStorage->wcsFree(statstg.pwcsName);
    }
  } while (status == S_OK);
  ...
  for (OMUInt32 i = 0; i < entries; i++) {
    ...
    case SF_STRONG_OBJECT_REFERENCE_VECTOR: {
      ...
      for (OMUInt32 entry = 0; entry < _count; entry++) {
        ...
        char number[256];
        sprintf(number, "%lx", (long unsigned int)vectorIndex[entry]._elementName);
    ...
    case SF_STRONG_OBJECT_REFERENCE_SET : {
      ...
      for (OMUInt32 entry = 0; entry < _count; entry++) {
        ...
        char number[256];
        sprintf(number, "%lx", (long unsigned int)setIndex[entry]._elementName);
...
void openStorage(char* fileName, IStorage** storage)
{
  ...
  result = _ssStorage->StgIsStorageFile(wcFileName);
  ...
  result = _ssStorage->StgOpenStorage(
    wcFileName,
    NULL,
      ...
      if (s == 0) {
        if (b) {
          if ((memcmp(&x, &aafFileKindAafSSBinary, sizeof(CLSID)) == 0) ||
              (memcmp(&x, &aafFileKindAaf4KSSBinary, sizeof(CLSID)) == 0) ||
