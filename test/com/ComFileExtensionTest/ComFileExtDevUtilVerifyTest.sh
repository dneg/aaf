...
# meant simply to verify that file extension limitations that do *not*
# exist in the SDK itself do not creep into any of the commonly used
# development utilities. The test simply verifies that the utilities
# will open the files without error. It does not test any other aspect
# of the utilities' operation. It uses the files generated by
# ComFileExtensionTest and is considered an adjunct to that test.
#
# The script is configured to run on unix platforms only. It is not
# executed as part of a "make check" build because to do so would
# introduce a dependency on each utility into check build.
#
# The utilities that tested are those that are commonly used for AAF software
...
#
# aafdump - low level AAF structured storage dump utility
#
